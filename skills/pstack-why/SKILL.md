...
Before spawning investigators, list the available MCPs from your environment. Use the available-tools map when present. Otherwise inspect whatever directory or config your harness exposes for enabled MCP servers.
...
Launch all matching investigators in parallel (as background tasks) so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

Subagent config (each): a read/write-capable subagent with MCP access. **Do not run investigators in a stripped-down readonly mode that disables MCP access.** The source control investigator would be safe without MCP access, but keep modes uniform. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
Spawn one synthesizer subagent, with MCP access enabled (its quality check spot-verifies citations, which can require MCP access).
...
---
Sourced from `cursor/plugins/pstack/skills/why`. Original text referenced Cursor's `Task` subagent tool (`subagent_type`, `readonly` mode, `model:` slugs) for spawning investigators/synthesizer; this copy generalizes that to "spawn a subagent with MCP access" so it maps onto this install's `task()` tool (use a `category` or `subagent_type`, `run_in_background: true`, and skip readonly/Ask-style modes since they strip MCP access). No functional content was removed, only the tool-invocation language.
