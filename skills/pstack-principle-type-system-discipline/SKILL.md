...
Applies to any typed language. This install's `typescript-best-practices` skill grounds it in TypeScript syntax specifically.
...
---
Sourced from `cursor/plugins/pstack/skills/principle-type-system-discipline`. This install's `typescript-best-practices` and `principle-boundary-discipline` skills both cross-reference this one; all three are installed together, so the set is self-contained.
