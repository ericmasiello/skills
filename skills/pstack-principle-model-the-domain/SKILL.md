...
---
Sourced from `cursor/plugins/pstack/skills/principle-model-the-domain`, installed under this name to match its original slug. **Note:** this is a coding-structure principle (state machines, discriminated unions, registries), distinct in scope from this install's existing `domain-modeling` skill, which maintains a project's glossary/ubiquitous-language doc and ADRs. The two are complementary, not overlapping, but the near-identical names can cause confusion when picking which one to invoke — this one is for shaping code, `domain-modeling` is for shaping documentation.
