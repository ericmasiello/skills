...
---
Sourced from `cursor/plugins/pstack/skills/principle-fix-root-causes`. Self-contained, no dependencies. Overlaps in intent with this install's existing `diagnosing-bugs` skill (both say: reproduce first, trace to root cause, resist symptom guards) — treat this as the atomic citation for that broader workflow rather than loading both for the same task.
