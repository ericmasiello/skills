...
---
Sourced from `cursor/plugins/pstack/skills/blast-radius`. Uses this install's `pstack-why` skill for step 1 (pulling PR/commit context) and its `pstack-why` for step-2 citation discipline. Step 6's `arena` escalation (multi-model merge for big/wide changes) was **not** installed as part of this package; skip that step or request the `arena` skill separately if you need it.
